package com.src.java.crm.web.admin;
...
import com.src.java.crm.services.GPSAdminService;
...
public class ModuleController extends HttpServlet {
	public void doGet(HttpServletRequest rq, HttpServletResponse rs) throws ServletException, IOException {
		...
		List<GpsModule> moduleList = (List<GpsModule>) context.getAttribute("GpsModuleList");
		...
		if (id != null && id.trim().length() > 0) {
			...
			GPSAdminService service = new GPSAdminService();
			service.removeModule(tid);
			for (GpsModule s : moduleList) {
				if (s.getId() == tid) {
					moduleList.remove(s);
	...
	public void doPost(HttpServletRequest rq, HttpServletResponse rs) throws ServletException, IOException {
		...
		if (!found) {
			GPSAdminService service = new GPSAdminService();
			GpsModule newG = service.saveAndGetModule(name);
			moduleList.add(newG);
